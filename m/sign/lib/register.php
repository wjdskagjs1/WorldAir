...
if(!preg_match("/^5\.2/", PHP_VERSION)){ //if(PHP_VERSION != "5.2.12")
   ...
   {            // 레코드 삽입 명령을 $sql에 입력
	    $sql = "insert into member(userid, password, nickname, email, introduce, regist_day, point, privilege, ip) ";
		$sql .= "values('$userid', '$password', '$nickname', '$email', '$introduce', '$regist_day', 100, false, '$ip')";
